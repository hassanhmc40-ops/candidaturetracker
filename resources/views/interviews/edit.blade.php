                    ...
                    <form method="POST" action="{{ route('interviews.update', $interview) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                            <div>
                                ...
                                <select id="type" name="type" class="mt-1.5 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @foreach($typeOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('type', $interview->type) == $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="result" name="result" class="mt-1.5 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($resultOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('result', $interview->result) == $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="scheduled_date" name="scheduled_date" type="date" class="mt-1.5 block w-full" :value="old('scheduled_date', $interview->scheduled_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('scheduled_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="scheduled_time" value="Time" />
                                <x-text-input id="scheduled_time" name="scheduled_time" type="time" class="mt-1.5 block w-full" :value="old('scheduled_time', $interview->scheduled_time)" required />
                        ...
                        <div class="mb-6">
                            <textarea id="preparation_notes" name="preparation_notes" rows="4" class="mt-1.5 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('preparation_notes', $interview->preparation_notes) }}</textarea>
                        ...
                        <div class="flex items-center justify-end gap-3 pt-5 border-t border-gray-100 dark:border-gray-800">
                            <a href="{{ route('applications.show', $interview->application) }}" class="inline-flex items-center px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg font-medium text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 transition-colors">
                                Cancel
                            </a>
