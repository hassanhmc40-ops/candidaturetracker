                    ...
                    <form method="POST" action="{{ route('applications.update', $application) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                            <div>
                                ...
                                <x-text-input id="company_name" name="company_name" type="text" class="mt-1.5 block w-full" :value="old('company_name', $application->company_name)" required />
                                <x-input-error :messages="$errors->get('company_name')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="job_title" value="Job Title" />
                                <x-text-input id="job_title" name="job_title" type="text" class="mt-1.5 block w-full" :value="old('job_title', $application->job_title)" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                            ...
                            <div>
                                ...
                                <select id="priority" name="priority" class="mt-1.5 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    @foreach($priorityOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('priority', $application->priority) == $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="application_date" name="application_date" type="date" class="mt-1.5 block w-full" :value="old('application_date', $application->application_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('application_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="job_url" value="Job Posting URL (optional)" />
                                <x-text-input id="job_url" name="job_url" type="url" class="mt-1.5 block w-full" :value="old('job_url', $application->job_url)" />
                        ...
                        <div class="mb-8">
                            <textarea id="notes" name="notes" rows="4" class="mt-1.5 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $application->notes) }}</textarea>
